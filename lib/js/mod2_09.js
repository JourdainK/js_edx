...
window.onload = function init() {
    ...
    canvas01 = document.querySelector("#canvasExample01");
    ctx01 = canvas01.getContext("2d");

    canvas01.addEventListener("mousemove", function(evt) {
        mousePos01 = getMousePos01(canvas01, evt);
        var message = 'Mouse position: ' + mousePos01.x + ',' + mousePos01.y;
        writeMessage(canvas01, message);
    });

    canvas01.addEventListener("mousedown", function(evt) {
        mousePos01 = getMousePos01(canvas01, evt);
        mouseButton01 = evt.button;
        var message = 'Mouse position: ' + mousePos01.x + ',' + mousePos01.y
        var pressed = document.querySelector("#buttonPressed01");
        pressed.innerHTML = "Button pressed: " + mouseButton01;
        writeMessage(canvas01, message);
    }, false);

    canvas01.addEventListener("mouseup", function(evt) {
        mousePos01 = getMousePos01(canvas01, evt);
        var message = 'Mouse position: ' + mousePos01.x + ',' + mousePos01.y;
        writeMessage(canvas01, message);
    },false);
...
function getMousePos01(canvas, evt) {
    // necessary work in the canvas coordinate system
    var rect = canvas.getBoundingClientRect();
    return {
        x: evt.clientX - rect.left,
